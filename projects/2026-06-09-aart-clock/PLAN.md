...
- `sg_phase` (build script) — the **off-grid phase specialist**: normalized looping phase at an arbitrary rate, decoupled from the grid, with offset. Has a **reset input** (`in1`) that re-zeros the phase on an incoming pulse — wiring `sg_clock`'s `pulse_bar` (etc.) into it locks the loop to the clock, making `clock → phase → map` a literal chain. Free-running when unwired. Outputs: `phase`, `ramp`, `pulse_wrap`. (Beat-relative *rate* sync deferred; reset-align covers chaining.)
